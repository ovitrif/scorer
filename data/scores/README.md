...
- `merged-selected-overlay.bin`: serialized scorer candidate generated by keeping
  source-z as baseline and overlaying only selected incoming source-b entries.
- `merged-selected-overlay.decoded.json`: decoded JSON view of
  `merged-selected-overlay.bin`.
- `merged-selected-overlay.report.json`: selected-overlay merge audit report.
...
cargo run --bin scorer-kit -- merge <source-z.bin> <source-b.bin> \
  ...
  --output data/scores/merged-selected-overlay.bin \
  --report data/scores/merged-selected-overlay.report.json
cargo run --bin scorer-kit -- decode data/scores/merged-selected-overlay.bin \
  --label merged-selected-overlay \
  --compact \
  --save data/scores/merged-selected-overlay.decoded.json
```
